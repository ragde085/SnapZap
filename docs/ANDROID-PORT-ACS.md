...
| **AC-2.6** | ✅ **PARTIALLY DONE** | SkiaSharp and SQLite both cleared (AC-2.3, AC-2.5) — neither is a v1 blocker any more. ONNX (AC-2.4) is still untested: the self-test does not score an image because no model is deployed to the device yet. |
